...
- **macOS 默认分辨率（如 1470×956 / Retina 内屏）屏幕被横向拉歪 / 撕裂（stride 未对齐）—— 根治（方案 C：原生 ScreenCaptureKit）** (`macos/Runner/ScreenCaptureKitManager.swift`, `macos/Runner/MainFlutterWindow.swift`, `macos/Runner.xcodeproj/project.pbxproj`, `lib/webrtc/screen_capture.dart`)：根因为 flutter_webrtc 内置 `getDisplayMedia`（闭源 `RTCDesktopCapturer`，走 WebRTC.framework 二进制，Dart 层无法改、且静默忽略 `exact` 尺寸约束）把屏幕按**原生分辨率**采集，并以 `width×4` 假设行步长做 I420 转换；当原生宽度不是 64 的倍数（如内屏 2940 → `2940×4=11760` 非 256 整数倍）时真实 `bytesPerRow` 带 padding，错位被**烘焙进原始 CVPixelBuffer**，任何 Dart 层重编码（含之前的预览回环）都救不了。现改为在原生层用 **ScreenCaptureKit `SCStream`** 直接采集：经 flutter_webrtc 的 `peerConnectionFactory` 自建 `RTCVideoSource`/`RTCVideoTrack`，让系统把屏幕**缩放到精确的 64 对齐宽 + 偶数高**输出，并每帧用 **`RTCCVPixelBuffer`（读取真实 `bytesPerRow` 而非假设 `width×4`）** 喂给 WebRTC，从源头消除 stride 错位；同时把该 track/stream 注册进 flutter_webrtc 的 `localTracks`/`localStreams`，对上层（`addTrack`、渲染器、`getSettings`）完全透明。`getDisplayMedia` 仅作为 macOS < 12.3 的回退保留。⚠️ 工程部署目标已从 10.15 提到 **12.3**（ScreenCaptureKit 最低要求），Runner 已新增并编译 `ScreenCaptureKitManager.swift`，并仍需 `com.apple.security.screen-recording` 授权。
...
- **Controller 本地预览在非 64 对齐宽度下仍歪扭（根因：错位烘焙在采集缓冲）** (`screen_capture.dart`, `peer_manager.dart`, `connection_manager.dart`, `control_screen.dart`)：屏幕画面被横向拉歪的根因是 macOS 采集缓冲的 `bytesPerRow` 按 256 字节对齐，而内屏原生宽度（如 2940）不是 64 的倍数 → `宽×4` 不等于行步长 → 错位被**烘焙进原始 CVPixelBuffer**。因此任何 Dart 层重新编码（含本地预览回环）都只是把"已歪的图"缩成更小的帧，**无法消除错位**——日志里 `renderer swap=YES`、`videoSize=1408x916` 却仍歪即印证此点。真正修复在**采集端**：把采集尺寸用 `exact` 约束请求为**宽度 64 对齐、高度偶数**（由 `_resolveCaptureSize` 按显示器物理分辨率保持比例算出，如 1408×916），逼 macOS 缩放到一个紧凑打包（bytesPerRow == 宽×4、无 256 padding）的帧，错位在源头消失；`exact` 不被支持时回退到无尺寸约束以免采集直接失败。回环（trickle ICE→非 trickle、预览流独立 id、VP8 优先）仍是必要的健壮层，但只对"已 stride-safe 的源"有效。
